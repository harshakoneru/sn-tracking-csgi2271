...
def get_mot_loader(dataset, test, data_dir="/scratch/hk3820/ByteTrack/datasets", workers=1, size=(800, 1440), ann_file_name = "test"):
    # Different dataset paths
    if dataset == "soccernet":
        direc = "SoccerNet"
        if test:
            name = "test"
            annotation = ann_file_name + ".json"
        else:
            name = "train"
...
#     subset = torch.utils.data.Subset(valdataset, [len(valdataset)-750, len(valdataset)])
